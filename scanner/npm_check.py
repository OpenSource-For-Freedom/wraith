...
COMPROMISED_PACKAGES: Dict[str, Dict] = {
    ...
    "node-ipc":              {"versions": ["10.1.1","10.1.2","9.2.2"],      "reason": "Protestware - wipes files on Russian/Belarusian IPs (CVE-2022-23812)"},
    "colors":                {"versions": ["1.4.44-liberty-2"],             "reason": "Protestware - infinite loop / corrupts output"},
    "faker":                 {"versions": ["6.6.6"],                        "reason": "Protestware - infinite loop (CVE-2022-21211)"},
    "ua-parser-js":          {"versions": ["0.7.29","0.8.0","1.0.0"],      "reason": "Cryptominer + credential stealer backdoor (CVE-2021-41265)"},
    "coa":                   {"versions": ["2.0.3","2.0.4","3.0.1","3.1.3"], "reason": "Backdoor - credential theft via postinstall"},
    "rc":                    {"versions": ["1.2.9"],                        "reason": "Backdoor - postinstall credential theft"},
    "event-source-polyfill": {"versions": ["1.0.31","1.0.32"],             "reason": "XSS data exfiltration"},
    "bootstrap-sass":        {"versions": ["3.3.7"],                       "reason": "Backdoor - exfiltrates env vars and npm tokens"},
    "eslint-scope":          {"versions": ["3.7.2"],                       "reason": "Credential theft - reads and exfils npm tokens"},
    "eslint-config-eslint":  {"versions": ["5.0.2"],                       "reason": "Malicious postinstall - npm credential exfil"},
    "getcookies":            {"versions": ["all"],                          "reason": "Hidden backdoor - cookie/credential theft"},
    "flatmap-stream":        {"versions": ["0.1.1"],                       "reason": "Bitcoin wallet stealer"},
    "event-stream":          {"versions": ["3.3.6"],                       "reason": "Contained flatmap-stream malicious dependency"},
    # 2022
    "styled-components":     {"versions": ["5.3.5-1"],                     "reason": "Typosquat version with backdoor"},
    "foreach":               {"versions": ["2.0.6"],                       "reason": "Hidden data exfiltration"},
    "cross-env":             {"versions": ["7.0.4"],                       "reason": "Compromised version - malicious postinstall"},
    # 2023
    "puppeteer":             {"versions": ["19.7.3"],                      "reason": "Malicious lookalike - cryptominer"},
    ...
    "discord.js-selfbot":    {"versions": ["all"],                         "reason": "Discord token stealer variant"},
    "axios-proxy":           {"versions": ["all"],                         "reason": "Typosquat of axios - data theft"},
    "nodemailer-callback":   {"versions": ["all"],                         "reason": "Credential exfiltration package"},
    "ip":                    {"versions": ["1.1.9","2.0.1"],               "reason": "SSRF vulnerability (CVE-2023-42282)"},
    # 2024
    "follow-redirects":      {"versions": ["1.15.5","1.15.4"],            "reason": "Credential exposure (CVE-2024-28849)"},
    "braces":                {"versions": ["3.0.2"],                       "reason": "ReDoS vulnerability (CVE-2024-4068)"},
    "tar":                   {"versions": ["6.1.12","6.1.13","6.1.14"],    "reason": "Path traversal (CVE-2024-28863)"},
    # 2025
    "lottie-player":         {"versions": ["all"],                         "reason": "Malicious package - crypto drainer payload"},
    "pdf-to-office":         {"versions": ["all"],                         "reason": "Backdoor - crypto wallet clipboard hijacker"},
    "@0xengine/xmlrpc":      {"versions": ["all"],                         "reason": "Crypto miner + data stealer - XMRig payload"},
    "ethers-provider":       {"versions": ["all"],                         "reason": "Crypto wallet drainer"},
    "solana-web3-adapter":   {"versions": ["all"],                         "reason": "Fake solana lib - wallet stealer"},
    "cline":                 {"versions": ["all"],                         "reason": "Suspected npm supply chain compromise (2025)"},
    "@cline/cline":          {"versions": ["all"],                         "reason": "Suspected npm supply chain compromise (2025)"},
    "vscode-cline":          {"versions": ["all"],                         "reason": "Related to cline supply chain incident (2025)"},
    ...
    "crossenv":              {"versions": ["all"],  "reason": "Typosquat of cross-env (CVE-2018-3728)"},
    "discordd":              {"versions": ["all"],  "reason": "Typosquat of discord.js - token stealer"},
    "discord-selfbot-v13":   {"versions": ["all"],  "reason": "Discord token stealer"},
    "electron-native-notify":{"versions": ["all"],  "reason": "Malicious postinstall - reverse shell"},
    # Cryptominers
    ...
    "@azure-sdk/http-client":{"versions": ["all"],  "reason": "Fake Azure package - cryptominer"},
    # Info stealers
    "ssb-gs":                {"versions": ["all"],  "reason": "SSH key exfiltration"},
    "linux-cpu-governor":    {"versions": ["all"],  "reason": "Credential/SSH key stealer"},
    "win-browser-settings":  {"versions": ["all"],  "reason": "Browser cookie/credential stealer"},
}
...
POPULAR_PACKAGES = [
    "lodash","moment","express","react","axios","webpack","babel","typescript",
    "eslint","prettier","jest","mocha","chai","nodemon","dotenv","cors",
    "mongoose","sequelize","pg","mysql2","redis","socket.io","next","vue",
    "angular","svelte","vite","rollup","parcel","gulp","rimraf","cross-env",
    "node-fetch","got","superagent","request","cheerio","puppeteer","playwright",
    "cypress","sinon","nyc","husky","lint-staged","lerna","nx","turbo",
]
    ...
    try:
        ...
        def check_node(name: str, node: dict):
            ...
            if resolved and not any(resolved.startswith(r) for r in ALLOWED_REGISTRIES):
                sev = "MEDIUM"
                if any(x in resolved.lower() for x in ["pastebin","ngrok","onion","discord","telegram"]):
                    sev = "CRITICAL"
                findings.append({
...
def run_npm_audit() -> List[Dict]:
    findings: List[Dict] = []
    roots = [os.environ.get("USERPROFILE",""), r"C:\dev", r"C:\projects", r"C:\src", r"C:\repos"]
    for root in roots:
        ...
        for item in Path(root).iterdir():
            ...
            try:
                r = subprocess.run(
                    ["npm", "audit", "--json"],
                    capture_output=True, text=True, timeout=45, cwd=str(item)
                )
                ...
                for vname, vdata in vulns.items():
                    ...
                    findings.append({
                        ...
                        "path":        str(item),
                        "reason":      vdata.get("title") or vdata.get("overview") or "Vulnerability found by npm audit",
                        "severity":    sev_raw,
                        "category":    "npm",
...
def scan_npm() -> Dict[str, Any]:
    ...
    search_paths = [
        ...
        r"C:\dev", r"C:\projects", r"C:\src", r"C:\repos", r"C:\workspace",
    ]
    try:
