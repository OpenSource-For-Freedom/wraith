...
def check_defender(findings: List[Dict]) -> None:
    try:
        ...
        SUSPICIOUS_EXCL = ["temp", "tmp", "appdata", "roaming", "downloads", "public", "recycl"]
        for excl in exclusion_paths:
            excl_str = str(excl)
            # Skip PowerShell permission-denied placeholder strings
            if "must be an administrator" in excl_str.lower() or excl_str.startswith("N/A"):
                continue
...
def check_smb(findings: List[Dict]) -> None:
    try:
        out = _ps("Get-SmbServerConfiguration | Select-Object EnableSMB1Protocol | ConvertTo-Json -Compress")
        if out:
            conf = json.loads(out)
...
def check_powershell_v2(findings: List[Dict]) -> None:
    try:
        out = _ps("Get-WindowsOptionalFeature -Online -FeatureName MicrosoftWindowsPowerShellV2Root 2>$null | "
...
def check_guest_account(findings: List[Dict]) -> None:
    try:
        out = _ps("Get-LocalUser -Name Guest | Select-Object Enabled | ConvertTo-Json -Compress")
        if out:
            data = json.loads(out)
        ...
        findings.append({
            ...
            "reason":   (
                ...
                + ("The password is stored in PLAINTEXT in the registry under DefaultPassword. "
                   ...
                   if pwd else
                   "Credentials are stored in LSA secrets (readable by SYSTEM-level attackers).")
            ),
            "severity": "CRITICAL",
            "category": "winsec",
...
if __name__ == "__main__":
    ...
    print(json.dumps({"scanner": "WRAITH-winsec", "mode": "winsec", "findings": results}, default=str, indent=2))
