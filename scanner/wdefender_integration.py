...
def check_active_threats() -> List[Dict]:
    ...
    STATUS_MAP = {0: "Unknown", 1: "Detected", 2: "Cleaned", 3: "Quarantined",
                  4: "Removed", 5: "Allowed", 6: "Blocked", 102: "CleanFailed",
                  103: "QuarantineFailed", 104: "RemoveFailed"}
    ...
    for threat in threats:
        ...
        resource_paths = "; ".join(str(r) for r in resources[:5]) if resources else "unknown"
        ...
        findings.append({
            ...
            "reason":      (
                ...
                + ("The threat is currently ACTIVE — malware may be running." if is_active else
                   "The threat has been detected but may not be fully remediated.")
...
def check_detection_history() -> List[Dict]:
    ...
    for detection in history[:50]:  # cap at 50
        ...
        det_str = det_time.strftime("%Y-%m-%d %H:%M:%S") if det_time else det_time_raw[:20]
        sev_label = SEVERITY_MAP.get(sev_id, str(sev_id))
        resource_paths = "; ".join(str(r) for r in resources[:3]) if resources else "unknown"
        ...
        findings.append({
            ...
            "reason":      (
                ...
                f"on {det_str}. "
                + ("Remediation FAILED — the threat may still be present." if not success
...
def check_protection_features() -> List[Dict]:
    ...
    # ── Network Protection ──
    net_prot = prefs.get("EnableNetworkProtection") or prefs.get("NetworkProtectionEnabled")
...
if __name__ == "__main__":
    sys.stderr.write("[WRAITH-DEFENDER] Windows Defender integration scan starting...\n")
    results = scan_defender()
    sys.stderr.write(f"[WRAITH-DEFENDER] Defender scan complete: {len(results)} findings\n")
    output = {
