...
C2_PORTS = {
    ...
    5555, 5554,              # Android debug / some RATs
    6666, 6667, 6668,        # IRC-based botnets
    7777,                    # Common RAT port
    8080, 8443, 8888,        # Alt-HTTP common C2 tunnels (flagged only with suspicious process)
    31337,                   # "Elite" legacy backdoor port
    12345, 54321,            # Legacy trojans
...
SUSPICIOUS_PROCESS_NAMES = {
    "powershell", "powershell_ise", "pwsh", "cmd", "wscript", "cscript",
    "mshta", "regsvr32", "rundll32", "certutil", "bitsadmin",
    "msiexec", "wmic", "installutil", "cmstp", "msbuild",
    "nc", "ncat", "netcat", "nmap", "socat",
    "python", "python3", "ruby", "perl",
}

TRUSTED_LISTENER_PROCESSES = {
    "svchost", "lsass", "system", "wininit", "services",
    "dns", "spoolsv", "searchindexer", "sqlservr", "mysqld",
    "httpd", "nginx", "node", "iisw3adm", "w3wp",
}
...
SENSITIVE_HOSTS = [
    ...
    "defender.microsoft.com", "smartscreen.microsoft.com",
        ...
        try:
            ...
            for line in r.stdout.splitlines():
                ...
                try:
                    ...
                    connections.append({
                        "LocalAddress": lhost, "LocalPort": int(lport),
                        "RemoteAddress": rhost, "RemotePort": int(rport),
                        "State": state, "OwningProcess": pid,
                    })
                except Exception:
                    pass
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout.strip():
            ...
            if isinstance(procs, dict):
                procs = [procs]
            return {p["Id"]: p.get("ProcessName", "") for p in procs if isinstance(p, dict)}
    except Exception as e:
    ...
    for conn in connections:
        ...
        if proc_name in SUSPICIOUS_PROCESS_NAMES and not _is_private(remote_ip):
            ...
            if key not in seen_remote_ips:
                ...
                sev = "CRITICAL" if proc_name in {"powershell", "pwsh", "mshta", "wscript", "cscript"} else "HIGH"
                findings.append({
...
"""
        ...
        for entry in listeners:
            ...
            if addr in ("0.0.0.0", "::") and port > 1024 and port not in {3389, 5985, 5986, 8080, 8443, 80, 443, 445}:
                if proc_name not in TRUSTED_LISTENER_PROCESSES and proc_name not in {"unknown", ""}:
                    findings.append({
        ...
        for hostname in hostnames:
            ...
            for sensitive in SENSITIVE_HOSTS:
                if sensitive in hostname_lower:
                    ...
                    reason = (
                        f"Hosts file overrides '{hostname}' → {ip}. "
        ...
        try:
            ...
            if proxy_enable:
                try:
                    ...
                    is_local = any(x in str(proxy_server) for x in
                    ...
                    findings.append({
                        ...
                        "reason":   (
                            ...
                            + ("A localhost proxy intercepts all HTTP/HTTPS traffic — "
                               ...
                               if is_local else
                               "An external proxy routes your traffic through a third-party server.")
                        ),
                        "severity": sev,
                        "category": "network",
...
def scan_dns_cache(findings: List[Dict]) -> None:
    ...
    try:
        ...
        dga_re = re.compile(r'^[a-z0-9]{12,}\.(?:com|net|org|info|biz|cc|pw|ru|cn|tk|top|xyz|gq|ml|cf|ga)$', re.I)
        suspicious_tld = re.compile(r'\.(tk|top|gq|ml|cf|ga|pw|cc|buzz|work|click|link|live|stream)$', re.I)
...
def scan_cert_store(findings: List[Dict]) -> None:
    ...
    try:
        ...
        TRUSTED_CA_KEYWORDS = {
            "microsoft", "digicert", "verisign", "entrust", "globalsign", "comodo",
            "sectigo", "baltimore", "godaddy", "thawte", "affirmtrust", "buypass",
            "usertrust", "starfield", "geotrust", "rapidssl", "cybertrust",
            "amazon", "lets encrypt", "isrg", "dst root", "comodoca",
            "certigna", "actalis", "trustwave", "swisssign", "quovadis",
            "deutschetelekom", "telekom", "telia", "secom", "taiwan", "accv",
            "cnnic", "certum", "asseco", "belgian", "chunghwa", "harica",
        }
...
if __name__ == "__main__":
    ...
    print(json.dumps({"scanner": "WRAITH-network", "mode": "network", "findings": results}, default=str, indent=2))
