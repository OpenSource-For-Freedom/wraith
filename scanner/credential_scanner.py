...
LSASS_DUMP_PATTERNS = [
    "lsass", "lsas", "lsa_dump", "lsadump", "memory.dmp",
    "minidump", "procdump", "full_dump",
]
...
PASSWORD_PATTERNS = [
    ...
    re.compile(r'connectionstring\s*=\s*["\']?.*password\s*=\s*([^;>"\']+)', re.IGNORECASE),
...
def check_credential_manager() -> List[Dict]:
    ...
    HIGH_RISK_TARGETS = [
        "domain:", "TERMSRV/", "MicrosoftOffice", "OneDriveMicros",
        "WindowsLive", "git:", "github", "bitbucket", "gitlab",
    ...
    for entry in entries:
        ...
        if any(target_lower.startswith(p) or p in target_lower for p in BENIGN_CM_PREFIXES_LOWER):
            continue
...
def check_ps_history() -> List[Dict]:
    ...
    default_history = userprofile / "AppData" / "Roaming" / "Microsoft" / \
    ...
    if users_dir.exists():
        for user_dir in users_dir.iterdir():
            h = user_dir / "AppData" / "Roaming" / "Microsoft" / "Windows" / \
        ...
        try:
            ...
            for pattern in PASSWORD_PATTERNS:
                for match in pattern.finditer(content):
                    ...
                    line_text  = content[line_start:line_end if line_end > 0 else len(content)].strip()
        ...
        try:
            for cfg_file in search_dir.rglob("*.config"):
                ...
                try:
                    ...
                    for pattern in PASSWORD_PATTERNS[:3]:  # only connection string patterns
...
if __name__ == "__main__":
    ...
    sys.stderr.write(f"[WRAITH-CRED] Credential scan complete: {len(results)} findings\n")
    output = {
