...
def test_emit_produces_well_formed_json(scanner_module, capsys):
    findings = [
        {"title": "x", "path": "", "reason": "y",
         "severity": "HIGH", "category": "yara", "subcategory": "match"},
    ]
    ...
    json_line = next(line for line in captured.splitlines()
...
def test_emit_severity_buckets_count_correctly(scanner_module, capsys):
    findings = [
        {"category": "x", "subcategory": "x", "severity": "CRITICAL", "title": "a", "path": "", "reason": ""},
        {"category": "x", "subcategory": "x", "severity": "CRITICAL", "title": "b", "path": "", "reason": ""},
        {"category": "x", "subcategory": "x", "severity": "HIGH",     "title": "c", "path": "", "reason": ""},
        {"category": "x", "subcategory": "x", "severity": "MEDIUM",   "title": "d", "path": "", "reason": ""},
        {"category": "x", "subcategory": "x", "severity": "INFO",     "title": "e", "path": "", "reason": ""},
    ]
    scanner_module.emit(findings, mode="all")
    captured = capsys.readouterr().out
    payload = json.loads(next(l for l in captured.splitlines() if l.startswith('{"scanner"')))
...
def test_emit_error_field_propagates(scanner_module, capsys):
    ...
    payload = json.loads(next(l for l in capsys.readouterr().out.splitlines()
...
def test_anomaly_score_high_for_critical_with_path(scanner_module):
    finding = {"severity": "CRITICAL", "category": "yara",
               "subcategory": "yara_match", "path": "/x", "reason": ""}
...
def test_anomaly_score_lower_for_info(scanner_module):
    finding = {"severity": "INFO", "category": "events",
               "subcategory": "info", "path": "", "reason": ""}
    score = scanner_module.compute_anomaly_score(finding)
    assert score < 50.0
...
def test_anomaly_score_within_bounds(scanner_module):
    ...
    samples = [
        {"severity": "CRITICAL", "category": "yara", "subcategory": "match", "path": "x", "reason": ""},
        {"severity": "INFO",     "category": "",     "subcategory": "",      "path": "",  "reason": ""},
        {"severity": "HIGH",     "category": "network", "subcategory": "c2_port", "path": "", "reason": "4444"},
    ]
    for f in samples:
...
def test_main_persistence_mode_invokes_scan_persistence(scanner_module, capsys, monkeypatch):
...
def test_main_all_mode_invokes_every_module(scanner_module, capsys, monkeypatch):
    ...
    targets = [
        "scan_persistence", "scan_yara", "scan_heuristics", "scan_events",
        "scan_npm", "scan_processes", "scan_network_module",
        "scan_winsec_module", "scan_rootkit_module", "scan_ads_module",
        "scan_browser_module", "scan_defender_module",
        "scan_credential_module", "scan_kev_module",
        "scan_vuln_drivers_module", "scan_tor_module", "scan_intel_module",
    ]
...
def test_main_vuln_drivers_mode_invokes_scanner(scanner_module, capsys, monkeypatch):
    ...
    with patch.object(scanner_module, "scan_vuln_drivers_module", return_value=[]) as m_vd:
        scanner_module.main()
